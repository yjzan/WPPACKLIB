(function(global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : typeof define === 'function' && define.amd ? define('uikit', factory) : global.bdtUIkit = factory()
    ...
    var startsWithFn = strPrototype.startsWith ||
        function(search) {
    ...
    var endsWithFn = strPrototype.endsWith ||
        function(search) {
    ...
    var escapeFn = window.CSS && CSS.escape ||
        function(css) {
    ...
    var touch = {},
        clickTimeout, swipeTimeout, tapTimeout, clicked, scrolled;
    ready(function() {
    ...
    function getComponentName(attribute) {
        return startsWith(attribute, 'bdt-') || startsWith(attribute, 'data-bdt-') ? camelize(attribute.replace('data-bdt-', '').replace('bdt-', '')) : false
    }
    ...
    var Togglable = {
        ...
        methods: {
            toggleElement: function(targets, show, animate$$1) {
                ...
                return new Promise(function(resolve) {
                    ...
                    if (!this$1.queued || !isUndefined(animate$$1) || !isUndefined(show) || !this$1.hasAnimation || targets.length < 2) {
                        ...
                        var el = toggled[0];
                        var inProgress = Animation.inProgress(el) && hasClass(el, 'bdt-animation-leave') || Transition.inProgress(el) && el.style.height === '0px';
                        p = all(toggled);
                        if (!inProgress) {
                            p = p.then(function() {
                                var p = all(untoggled);
            ...
            _toggleElement: function(el, show, animate$$1) {
                var this$1 = this;
                show = isBoolean(show) ? show : Animation.inProgress(el) ? hasClass(el, 'bdt-animation-leave') : Transition.inProgress(el) ? el.style.height === '0px' : !this.isToggled(el);
        ...
        events: [{
            ...
            handler: function(ref) {
                ...
                if (current === this.input) {
                    toggleClass(this.state, ("bdt-" + (includes(type, 'focus') ? 'focus' : 'hover')), includes(['focusin', 'mouseenter'], type))
                }
            }
        }, {
            name: 'change',
    ...
    var closeIcon = "<i class=\"iconfont icon-icon_roundclose\" aria-hidden=\"true\"></i>";
    ...
    var Search = {
        ...
        computed: {
            icon: function(ref, $el) {
                var icon = ref.icon;
                return hasClass($el, 'bdt-search-icon') && parents($el, '.bdt-search-large').length ? 'search-large' : parents($el, '.bdt-search-navbar').length ? 'search-navbar' : icon
            }
        }
    };
    var Close = {
        ...
        update: {
            read: function(ref) {
                ...
                if (image || !this.target.some(function(el) {
                        return isInView(el, this$1.offsetTop, this$1.offsetLeft)
                    })) {
                    if (!this.isImg && image) {
    ...
    function install$1(bdtUIkit) {
        ...
        bdtUIkit.modal.alert = function(message, options) {
            ...
            options = assign({
                bgClose: false,
                escClose: false,
                labels: bdtUIkit.modal.labels
            }, options);
            return new Promise(function(resolve) {
                return on(bdtUIkit.modal.dialog((" <div class=\"bdt-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"bdt-modal-footer bdt-text-right\"> <button class=\"bdt-button bdt-button-primary bdt-modal-close\" autofocus>" + (options.labels.ok) + "</button> </div> "), options).$el, 'hide', resolve)
            })
        };
        ...
        methods: {
            ...
            show: function(item) {
                ...
                for (var i = 0; i < length; i++, next = (next + dir + length) % length) {
                    if (!matches(this.toggles[next], '.bdt-disabled, [disabled]')) {
    ...
    var Tab = {
        ...
        connected: function() {
            var cls = hasClass(this.$el, 'bdt-tab-left') ? 'bdt-tab-left' : hasClass(this.$el, 'bdt-tab-right') ? 'bdt-tab-right' : false;
            if (cls) {
    ...
    var LightboxPanel = {
        ...
        events: [{
            ...
            handler: function(_, item) {
                ...
                var matches$$1;
                if (type === 'video' || source.match(/\.(mp4|webm|ogv)($|\?)/i)) {
    ...
    var Notification = {
        ...
        created: function() {
            ...
            this.$mount(append(container, ("<div class=\"" + (this.clsMsg) + (this.status ? (" " + (this.clsMsg) + "-" + (this.status)) : '') + "\"> <a href=\"#\" class=\"" + (this.clsClose) + "\" data-bdt-close></a> <div>" + (this.message) + "</div> </div>")))
        },
        connected: function() {
            var this$1 = this;
            var obj;
    ...
    var SliderParallax = {
        ...
        events: [{
            ...
            handler: function(ref) {
                ...
                Transition.start(this.$el, this.getCss(isIn(type) ? .5 : dir > 0 ? 1 : 0), duration, timing).
                catch (noop)
            }
        }, {
